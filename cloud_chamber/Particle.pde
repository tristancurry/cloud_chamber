class Particle {
  ...
  void update(float _BField, float _EField, float _density){    //method to update position, velocity and acceleration of particle
    ...
    velX = velX - _density*pow(abs(charge),12)*velX/mass;
    velY = velY -_density*pow(abs(charge),12)*velY/mass;
    velZ = velZ - _density*pow(abs(charge),12)*velZ/mass;
  ...
  void display(){
    noStroke();
    fill(255,0,0, 255*(100 - posZ)/100);
    if(energy > 1.0 && mass > 0){
      ellipse(posX,posY,0.0*fftLog.getAvg(10),0.0*fftLog.getAvg(10));
    }
