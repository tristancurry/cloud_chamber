...
void draw(){
  ...
  if(mousePressed){
  if(mouseButton == LEFT){
    Particle newParticle = new Particle(mouseX, mouseY, -1,1,1, random(50,150),random(PI/16, PI/12), random(0,TWO_PI),true);
    particleList.add(newParticle);
  } else if(mouseButton == RIGHT){
    Particle newParticle = new Particle(mouseX, mouseY, -1,1,-1, random(50,100),random(PI/16, PI/12), random(0,TWO_PI),false);
    particleList.add(newParticle);
  }
  }
  if(random(0,2)>1.96){
  ...
  if(particleList.size() != 0){
    for(int i = 0; i < particleList.size(); i++){
      ...
      trailDroplets(thisParticle.posX,thisParticle.posY,thisParticle.posZ, thisParticle.charge);
       if(thisParticle.energy < 0.88*4000  && thisParticle.charge == 2){
        particleList.remove(i);
       } else if(thisParticle.energy < 0.88||thisParticle.posZ < -3 || thisParticle.posZ > 70){
        particleList.remove(i);
      ...
      } else if(random(0,2)>1.70 && thisParticle.charge == 1 && thisParticle.isAnti){
        ...
        particleList.remove(i);
      } else if(random(0,1)>0.20 && thisParticle.charge == 0 && thisParticle.mass == 0 ){
        thisParticle.computeAltAzi();
